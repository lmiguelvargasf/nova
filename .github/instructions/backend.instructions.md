...
- Pagination (lists):
  ...
  - Use Litestar’s `AbstractAsyncCursorPaginator` and return `{ items: [...], page: { next_cursor, limit, has_next } }`.
...
6. REST list pagination (when adding REST list endpoints)
   - Use cursor pagination with `limit` + optional `cursor` and the `{ items, page }` response envelope.
7. GraphQL mutation (`graphql/mutations.py`)
   ...
   - Call `db_session.rollback()` on errors; use `auto_commit=True` for single-operation mutations.
