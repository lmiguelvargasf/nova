---
name: prune-no-graphql
description: Prune GraphQL components and keep REST stack.
---

Task:

- Run `task template:prune:no-graphql`.
- Run `task template:prune:validate -- no-graphql`.
- Create branch/commit/push using Taskfile wrappers:
  - `task git:branch -- codex/nova-prune-no-graphql`
  - `task git:commit -- "refactor: prune template to no-graphql"`
  - `task git:push`
- Create PR with `.github/prompts/pr-creation.prompt.md`.

Constraints:

- Do not run raw destructive commands; Taskfile targets are the source of truth.
- Stop if prune guardrails fail (dirty tree, main branch, confirmation denied).

Output:

- List files changed.
- Include commands executed.
- Include PR URL.
