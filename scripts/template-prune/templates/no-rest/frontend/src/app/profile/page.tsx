...
export default function ProfilePage() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const result = await updateCurrentUser({ variables: { userInput } });
    ...
    content = (
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="border-t border-slate-200 pt-4">
          <p className="text-xs font-semibold uppercase tracking-[0.2em] text-slate-500">Danger zone</p>
          <p className="mt-2 text-sm text-slate-600">This action can be reversed by logging in again.</p>
