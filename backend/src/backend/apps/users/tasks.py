...
@app.task
def deactivate_inactive_users(cutoff_days: int | None = None) -> int:
    if cutoff_days is None:
        cutoff_days = settings.celery_inactive_cutoff_days

    return asyncio.run(_deactivate_inactive_users_async(cutoff_days))


async def _deactivate_inactive_users_async(cutoff_days: int) -> int:
